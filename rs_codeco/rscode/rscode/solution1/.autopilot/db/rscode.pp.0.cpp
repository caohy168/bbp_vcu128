...
__attribute__((sdx_kernel("encode_rs", 0))) void encode_rs(int data_in[235],int bb_out[255 -235],int recd_out [255],
...
{_ssdm_SpecArrayDimSize(data_in, 235);_ssdm_SpecArrayDimSize(bb_out, 20);_ssdm_SpecArrayDimSize(recd_out, 255);_ssdm_SpecArrayDimSize(alpha_to_out, 256);_ssdm_SpecArrayDimSize(index_of_out, 256);_ssdm_SpecArrayDimSize(gg_out, 21);
#pragma HLS INTERFACE axis register register_mode=both port=data_in
# 14 "rscode.cpp"

#pragma HLS INTERFACE axis register register_mode=both port=recd_out
# 14 "rscode.cpp"

#pragma HLS TOP name=encode_rs
# 14 "rscode.cpp"
 ...
 int i,j ;
 int feedback ;
 VITIS_LOOP_20_1: for(i=0; i<235; i++)
  data[i] = data_in[i];
 VITIS_LOOP_22_2: for (i=0; i<255 -235; i++) bb[i] = 0 ;
 VITIS_LOOP_23_3: for (i=235 -1; i>=0; i--)
    {

  feedback = index_of[data[i]^bb[255 -235 -1]] ;
  if(feedback != -1)
        {
   VITIS_LOOP_29_4: for (j=255 -235 -1; j>0; j--)
            if (gg[j] != -1)
   ...
   bb[0] = alpha_to[(gg[0]+feedback)%255] ;
        ...
        {
   VITIS_LOOP_38_5: for (j=255 -235 -1; j>0; j--)
    bb[j] = bb[j-1] ;
   bb[0] = 0 ;
        };
    };

 VITIS_LOOP_44_6: for (i=0; i<255 -235; i++) bb_out[i] = bb[i];


 VITIS_LOOP_47_7: for (i=0; i<255 -235; i++)
  recd_out[i] = bb[i] ;
 VITIS_LOOP_49_8: for (i=0; i<235; i++)
...

{
 ...
 mask >>= 1;
 VITIS_LOOP_70_2: for (i=8 +1; i<255; i++)
 {
 ...
 index_of[0] = -1 ;
 VITIS_LOOP_79_3: for (i=0; i<255; i++)
...

{
 ...
 gg[0] = 2 ;
 gg[1] = 1 ;
 VITIS_LOOP_90_1: for (i=2; i<=255 -235; i++)
    {
  gg[i] = 1 ;
  VITIS_LOOP_93_2: for (j=i-1; j>0; j--)
   if (gg[j] != 0)
  ...
  gg[0] = alpha_to[(index_of[gg[0]]+i)%255] ;
   ...
   VITIS_LOOP_105_3: for (i=0; i<=255 -235; i++){
    gg[i] = index_of[gg[i]];}
   VITIS_LOOP_107_4: for (i=0; i<=255 -235; i++){
       gg_out[i]=gg[i];}
}
