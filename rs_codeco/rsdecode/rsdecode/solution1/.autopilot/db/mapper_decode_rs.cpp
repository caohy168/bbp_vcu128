...
extern "C" void decode_rs(volatile void *, volatile void *, int*, int, int*, int*, int*);
extern "C" void apatb_decode_rs_hw(volatile void * __xlx_apatb_param_recd_in, volatile void * __xlx_apatb_param_recd_out, volatile void * __xlx_apatb_param_recd_gf_out, int __xlx_apatb_param_syn_error_out, volatile void * __xlx_apatb_param_alpha_to_out, volatile void * __xlx_apatb_param_index_of_out, volatile void * __xlx_apatb_param_gg_out) {
  // Collect __xlx_recd_gf_out__tmp_vec
  vector<sc_bv<32> >__xlx_recd_gf_out__tmp_vec;
  for (int j = 0, e = 255; j != e; ++j) {
    __xlx_recd_gf_out__tmp_vec.push_back(((int*)__xlx_apatb_param_recd_gf_out)[j]);
  }
  int __xlx_size_param_recd_gf_out = 255;
  int __xlx_offset_param_recd_gf_out = 0;
  int __xlx_offset_byte_param_recd_gf_out = 0*4;
  int* __xlx_recd_gf_out__input_buffer= new int[__xlx_recd_gf_out__tmp_vec.size()];
  for (int i = 0; i < __xlx_recd_gf_out__tmp_vec.size(); ++i) {
    __xlx_recd_gf_out__input_buffer[i] = __xlx_recd_gf_out__tmp_vec[i].range(31, 0).to_uint64();
  }
  ...
  // DUT call
  decode_rs(__xlx_apatb_param_recd_in, __xlx_apatb_param_recd_out, __xlx_recd_gf_out__input_buffer, __xlx_apatb_param_syn_error_out, __xlx_alpha_to_out__input_buffer, __xlx_index_of_out__input_buffer, __xlx_gg_out__input_buffer);
// print __xlx_apatb_param_recd_gf_out
  sc_bv<32>*__xlx_recd_gf_out_output_buffer = new sc_bv<32>[__xlx_size_param_recd_gf_out];
  for (int i = 0; i < __xlx_size_param_recd_gf_out; ++i) {
    __xlx_recd_gf_out_output_buffer[i] = __xlx_recd_gf_out__input_buffer[i+__xlx_offset_param_recd_gf_out];
  }
  for (int i = 0; i < __xlx_size_param_recd_gf_out; ++i) {
    ((int*)__xlx_apatb_param_recd_gf_out)[i] = __xlx_recd_gf_out_output_buffer[i].to_uint64();
  }
